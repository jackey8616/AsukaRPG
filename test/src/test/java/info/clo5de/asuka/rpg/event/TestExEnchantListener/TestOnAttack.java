...
public class TestOnAttack {
    ...
    private static AsukaPlayer attacker;
    ...
    public static void setup () throws Exception {
        ...
        attacker = mock(AsukaPlayer.class);
        defender = mock(LivingEntity.class);

        mockMap = spy(new HashMap<>());

        when(attacker.getExEnchantMap()).thenReturn(mockMap);
        when(attacker.getVanillaPlayer()).thenReturn(player);
        when(edbee.getEntity()).thenReturn(defender);
        when(edbee.getDamage()).thenReturn(damage);
    }

    @Test
    public void testOnAttack () {
        eel.onAttack(attacker, edbee);
    }
}
